...
class BalanceTunerTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        self.tiltbias_line, = self.ax.plot([], [], color="#e377c2", ls=":",  label="tilt_bias", lw=1.5)
    ...
    def update_tab(self):
        ...
        if snap["t"]:
            ...
            # Push cascade state to header
            cs_raw = snap["cascade_state"]
            if cs_raw:
                cs_names = ["DRIVING", "RAMPDOWN", "HOLDING"]
                cs_idx   = int(cs_raw[-1])
                self.app.cascade_var.set(f"State: {cs_names[cs_idx]}")
...
class BipedTunerApp(tk.Tk):
    def __init__(self):
        ...
        self.cascade_var      = tk.StringVar(value="State: --")
    ...
    def _build_header(self):
        ...
        ttk.Label(top, textvariable=self.cascade_var,
                  font=("Consolas", 10, "bold"), foreground="#e377c2").pack(side=tk.RIGHT, padx=10)
